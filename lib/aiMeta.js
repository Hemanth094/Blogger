...
export async function generateSEOMeta(title, content) {
  ...
  if (!plainText) {
    ...
    return {
      ...
      metaDescription: `Discover more about ${title} on ${siteName}. High-quality articles and insights.`,
...
function generateSimpleSummary(title, text, siteName = "BLOGGER") {
  ...
  const prefix = `Explore the full story on ${title}: `;
  const suffix = `... Read more on ${siteName}.`;
  const availableSpace = 160 - prefix.length - suffix.length;

  // If title is too long, we might have very little space for text
  if (availableSpace < 40) {
    return smartTruncate(`Explore ${title} and more professional insights on ${siteName}.`, 160);
  }

  const teaser = smartTruncate(cleanText, availableSpace).replace(/\.\.\.$/, "");
